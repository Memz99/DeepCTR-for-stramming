...
def parse_config():
    ...
    # train param
    if cfg['is_local']:
        cfg['train_buffer_size'], cfg['eval_buffer_size'], cfg['num_workers'] = 1000, 100, 3
...
def main(argv):
    ...
    if cfg['do'] == 'train':
        ...
        train(cfg, model, optims, train_loader)

    if cfg['do'] == 'eval':
        checkpoint = torch.load(cfg['checkpoint_load_path'])
        model.load_state_dict(checkpoint['model'])
        eval_files = sorted([os.path.join(cfg['eval_path'], file) for file in os.listdir(cfg['eval_path'])])
        eval_loader = get_dataset(eval_files,
                                  encoder_items,
                                  ret_idxs,
                                  label_feature_info,
